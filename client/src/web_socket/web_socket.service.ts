...
import { SocketEventEnum } from './dto/socket_event.enum';
import { SocketMessagemodel } from './dto/socket_message.dto';
import { WebSocketInstance } from './dto/web_socket_instance.dto';
...
export class WebSocketService implements OnModuleDestroy {
    ...
    private setupWebSocketEvents(instance: WebSocketInstance) {
        ...
        instance.webSocket.on('message', async (message) => {
            ...
            this.appService.handleInputSocketMessage(instance.url, JSON.parse(message))
    ...
    private handleReconnect(wsUrl: string) {
        ...
        if (instance) {
            if (instance.reconnectInterval <= this.maxReconnectInterval) {
                setTimeout(() => {
                    this.logger.debug(`Attempting to reconnect ${wsUrl} in ${instance.reconnectInterval}ms`);
                    this.createWebSocket(wsUrl);
                    instance.reconnectInterval *= 2; // Exponential backoff
                }, instance.reconnectInterval);
            } else {
                this.logger.error(`Max reconnect interval reached for ${wsUrl}. Could not reconnect.`);
    ...
    public sendMessage(wsUrl: string, message: string) {
        ...
        if (instance && instance.webSocket.readyState === WebSocket.OPEN) {
            ...
            this.logger.error(`WebSocket is not open for ${wsUrl}. Unable to send message.`);
        }
    }
