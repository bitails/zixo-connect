...
export class BlockService {
    ...
    async getMerkleRoot(blockHeight: number): Promise<string> {
        if (this.appConfigService.GENERATE_MERKLE_ROOT_LOCAL) {
            ...
            const rawBlockHeader: BlockHeaderModel = await this.getBlockByHeader(blockHeight);
    ...
    async validateSourceOfTransaction(rawTx: string, merklePath: BranchModel[]): Promise<boolean> {
