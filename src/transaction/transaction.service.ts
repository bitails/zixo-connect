...
import { ReqUserDataModel } from 'types/request';
...
export class TransactionService {
  ...
  verifyInputsTransaction(
    currentTransactionRaw: string,
    inputsRaw: string[],
  ): boolean {
    const current: TransactionModel = this.parseRawTransaction(
      currentTransactionRaw,
    );
    Logger.debug('Current Transaction:', JSON.stringify(current));

    const mergeOutputs: Vout[] = inputsRaw.flatMap(
      (input) => this.parseRawTransaction(input).vout,
    );
